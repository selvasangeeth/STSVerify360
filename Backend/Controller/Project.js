...
const getProject = async (req, res) => {
  try {
    ...
    if (role == 'admin' || role == 'user') {
      projects = await ProjectDetails.find({ assignedTo: userId }).select('projectName projectLogo _id');

    }
    else {
      // superAdmin
      projects = await ProjectDetails.find().select('projectName projectLogo _id');
    }
  ...
  try {
    ...
    try {
      await log.create({
        ...
        projectId: projectId,
        details: ` Project Deleted : ${projectName.projectName}`
